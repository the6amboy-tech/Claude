...
test("the live app shell uses one release version", () => {
  for (const asset of ["style.css", "apple-player.css", "app.js", "motion.js", "manifest.webmanifest"]) {
    assert.match(index, new RegExp(`${asset.replace(".", "\\.")}\\?v=25`));
    assert.match(worker, new RegExp(`${asset.replace(".", "\\.")}\\?v=25`));
  }
  assert.match(worker, /const CACHE = "asharas-v25"/);
...
test("mobile keeps five primary tabs and exposes stats inside Library", () => {
  const css = readFileSync(new URL("../apple-player.css", import.meta.url), "utf8");
  assert.match(css, /#side-listening,[\s\S]*display: none !important/);
  assert.match(index, /id="mobile-listening">Listening time/);
  assert.match(app, /mobileListening[\s\S]*showView\("stats"\)/);
});
